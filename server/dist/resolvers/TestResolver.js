...
export const resolvers = {
    ...
    Mutation: {
        ...
        updateItem: async (parent, { input, id }, context, info) => {
            let error = {};
            const updatedChat = await TestModel.findByIdAndUpdate({ _id: id }, { title: input.title, desc: input.desc }, { new: true })
                .catch(e => {
        ...
        deleteItem: async (parent, { input, id }, context, info) => {
            let error = {};
            const updatedChat = await TestModel.findByIdAndDelete({ _id: id })
                .catch(e => {
                console.log("is delete error ======", e);
                error = e;
                return false;
